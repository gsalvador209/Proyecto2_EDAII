...
public class ArbolBin {
 ...
 /*
    /**
     * Intercambia los nodos A y B. 
     * @param a El Nodo A
     * @param b El Nodo B
     *
    public void swap(Nodo a, Nodo b){
        int temp = b.getValue();
        b.valor = a.valor;
        a.valor = temp;
    ...
    void DFSUtil(Nodo v, List<Integer> visited) { 
        visited.add(v.valor);
        System.out.print(v.valor + " ");
        if(v.izq!=null){//&&(!visited.contains(v.izq.valor))){
            DFSUtil(v.izq,visited);
        }
        if(v.der!=null){//&&(!visited.contains(v.der.valor))){
            DFSUtil(v.der,visited);
        }
    }
...
/**
     *
     * @return Regresa la cantidad de nodos en el arbol.
     *
    public int size(){
        int tamaño=0;
        Nodo r = root;
